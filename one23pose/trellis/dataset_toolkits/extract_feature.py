...
def get_data(frames, sha256):
    with ThreadPoolExecutor(max_workers=1) as executor:
        def worker(view):
            image_path = os.path.join(opt.output_dir, f'render_{subset_name}', sha256, view['file_path'].split('/')[-1])
            try:
...
if __name__ == '__main__':
    ...
    subset_name = opt.subset_name
    feature_name = opt.model
    os.makedirs(os.path.join(opt.output_dir, f'features_{subset_name}', feature_name), exist_ok=True)

    # load model
    dinov2_model = torch.hub.load(os.path.join(torch.hub.get_dir(), 'facebookresearch_dinov2_main'), opt.model, source='local',pretrained=True)
    ...
    # get file list
    if os.path.exists(os.path.join(opt.output_dir, f'metadata.csv')):
        metadata = pd.read_csv(os.path.join(opt.output_dir, f'metadata.csv'))
    else:
        raise ValueError(f'metadata.csv not found')
    ...
    valid_sha256s = []
    render_dir = os.path.join(opt.output_dir, f'render_{subset_name}')
    
    def check_transforms(sha256):
        return sha256, os.path.exists(os.path.join(render_dir, sha256, 'transforms.json'))
    with ThreadPoolExecutor(max_workers=64) as executor:
        futures = [executor.submit(check_transforms, sha256) for sha256 in sha256s]
        for future in tqdm(as_completed(futures), total=len(futures), desc="Validating transforms.json"):
            sha256, exists = future.result()
            if exists:
                valid_sha256s.append(sha256)
    sha256s = valid_sha256s
    ...
    try:
        with ThreadPoolExecutor(max_workers=8) as loader_executor, \
            ...
            def loader(sha256):
                try:
                    with open(os.path.join(opt.output_dir, f'render_{subset_name}', sha256, 'transforms.json'), 'r') as f:
            ...
            def saver(sha256, pack, patchtokens, uv):
                ...
                save_path = os.path.join(opt.output_dir, f'features_{subset_name}', feature_name, f'{sha256}.npz')
