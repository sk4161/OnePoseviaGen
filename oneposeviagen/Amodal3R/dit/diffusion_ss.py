...
class GaussianDiffusion:
    ...
    def q_sample_linear(self, x_start, t, noise=None):
        """
        Linearly interpolate x_t = (1 - t) * x_0 + t * ϵ
        """
        if noise is None:
            noise = torch.randn(x_start.shape, device=x_start.device)
        assert noise.shape == x_start.shape

        t = t.view(-1, *([1] * (len(x_start.shape) - 1)))  # (B, 1, 1, ...)
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, y=None):
        ...
        data_t = self.q_sample_linear(x_start=data_start, t=t, noise=noise)

        eps_recon = denoise_fn(data_t, t, y)
        assert data_t.shape == data_start.shape
        # assert eps_recon.shape == torch.Size([B, N, 3])
        assert eps_recon.shape == data_start.shape
        target = noise - data_start
        losses = ((eps_recon - target)**2).mean(dim=list(range(1, len(data_start.shape))))
