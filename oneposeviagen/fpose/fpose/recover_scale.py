...
def get_all_pose(
    test_scene_dir, mesh, topic, debug, track_refine_iter=8, est_refine_iter=5
):
    set_logging_format()
    set_seed(0)
    ...
    os.system(
        f"rm -rf {debug_dir}/* && mkdir -p {debug_dir}/track_vis {debug_dir}/{topic}"
    )
...
def get_single_pose(
    raw_img, depth, mask, intrinsic, mesh, topic, num, debug, est_refine_iter=5
):
    set_logging_format()
    set_seed(0)
    code_dir = os.path.dirname(os.path.realpath(__file__))
    ...
    pose = est.register(
        K=K, rgb=color, depth=depth, ob_mask=mask, iteration=est_refine_iter
    )
...
def recover_scale(
    ...
    # 进行模型缩放比例和位姿的确定
    M, intrinsic_numpy, scale = get_scale(
