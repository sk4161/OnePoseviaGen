        ...
        for t in theta:
            for up in up_vectors:
                ...
                R, T = look_at_view_transform(
                    radius, t, p, False, position, lookat, up, device="cuda:0"
                )
...
def estimate_pose(
    ...
    plot_image_with_points(
        depths[best_pose], image_points, os.path.join(out_dir, "points_on_2D.png")
    )

    # Matched points on original picture
    match_points_on_mask = match_result["keypoints1"][
        match_result["matches"][match_result["matches"] > -1]
    ]
    ...
    match_points_on_raw = match_points_on_mask * np.array(
        [sclae_x, sclae_y]
    ) + np.array([mask_box[1], mask_box[0]])
    plot_image_with_points(
        raw_img, match_points_on_raw, os.path.join(out_dir, "points_original.png")
    )
    ...
    world_2_cam_render[:, :2] = -world_2_cam_render[
        :, :2
    ]  # change due to pytorch3D setting
    # print('Optimized pose for rendering', np.array2string(world_2_cam_render, separator=', '))
    color, _ = render_image(
        mesh_file, world_2_cam_render, raw_img.shape[1], raw_img.shape[0], fov, device
    )
    ...
    mesh_points = (
        np.hstack((world_points, np.ones((world_points.shape[0], 1)))) @ (world_2_cam).T
    )
    ...
    result = minimize(
        objective,
        initial_scale,
        args=(mesh_points, pcd_points, plane_model),
        method="L-BFGS-B",
    )
...
def get_intrinsic(intrinsic_file):
    if intrinsic_file.endswith(".json"):
        ...
        if len(intrinsic_data) != 7:
            ...
            intrinsic_tensor = o3d.core.Tensor([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])

            intrinsic_numpy = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
    else:
...
def rgbd_to_pointcloud(rgb_file, depth_file, intrinsic, out_dir):
    ...
    pcd = o3d.t.geometry.PointCloud.create_from_rgbd_image(
        rgbd, intrinsics=intrinsic, depth_scale=1000, depth_max=10.0
    )
...
if __name__ == "__main__":
    ...
    M = get_scale(
        mesh_file, depth_file, raw_img, ref_img, mask_file, out_dir, intrinsic_file
    )
