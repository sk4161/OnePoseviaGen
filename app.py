...
try:
    from app_3rd.sam_utils.inference import (
        SamPredictor,
        get_sam_predictor,
        run_inference,
    )
    from app_3rd.spatrack_utils.infer_track import (
        get_points_on_a_grid,
        get_tracker_predictor,
        run_tracker,
    )
...
tracker_model_offline = Predictor.from_pretrained(
    "checkpoints/OnePoseViaGen/SpatialTrackerV2/tracker_offline"
)
tracker_model_offline.eval()
tracker_model_online = Predictor.from_pretrained(
...
trellis_pipeline = Amodal3RImageTo3DPipeline.from_pretrained(
...
hi3dgen_pipeline = TrellisImageTo3DPipeline.from_pretrained(
...
def load_video_with_opencv(video_path):
    ...
    video_tensor = (
        torch.from_numpy(video_array).permute(0, 3, 1, 2).float()
    )  # (T, C, H, W)
...
def gpu_run_tracker(
    ...
    if tracker_model_arg is None or tracker_viser_arg is None:
        ...
        if mode == "offline":
            tracker_model_arg, tracker_viser_arg = get_tracker_predictor(
                out_dir, vo_points=vo_points, tracker_model=tracker_model_offline.cuda()
            )
        else:
            tracker_model_arg, tracker_viser_arg = get_tracker_predictor(
                out_dir, vo_points=vo_points, tracker_model=tracker_model_online.cuda()
            )
    ...
    query_xyt = (
        torch.cat([torch.zeros_like(grid_pts[:, :, :1]), grid_pts], dim=2)[0]
        .cpu()
        .numpy()
    )
    ...
    with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        data_npz_load["coords"] = (
            torch.einsum(
                "tij,tnj->tni", c2w_traj[:, :3, :3].cpu(), track3d_pred[:, :, :3].cpu()
            )
        ...
        intrinsics_dict = {
            str(frame_id): data_npz_load["intrinsics"][
                frame_id
            ].tolist()  # 转为 list 才能被 json 序列化
...
def process_point_cloud_data(npz_file, width=256, height=192, fps=4):
    ...
    rgb_video = np.stack(
        [
            cv2.resize(frame, fixed_size, interpolation=cv2.INTER_AREA)
    ...
    depth_video = np.stack(
        [
            cv2.resize(frame, fixed_size, interpolation=cv2.INTER_NEAREST)
    ...
    for t in range(T):
        homogeneous_trajs = np.concatenate(
            [trajs[t], np.ones((trajs.shape[1], 1))], axis=1
        )
...
def handle_video_upload(video, fps):
    ...
    print(
        f"🎬 Video settings for '{video_name}': grid_size={grid_size_val}, vo_points={vo_points_val}, fps={fps_val}"
    )
...
def select_point(original_img: str, sel_pix: list, evt: gr.SelectData, objects):
    ...
    try:
        ...
        original_img_array = base64_to_numpy(
            frame_data["data"], frame_data["shape"], frame_data["dtype"]
        )
        ...
        if object_id not in objects:
            objects[object_id] = {
                "points": [],
                "mask": None,
                "color": plt.get_cmap("tab10")(len(objects) % 10)[:3],
            }
...
def transform_point(
    point, original_size, target_size=518, mode="crop", keep_ratio=False
):
    """
...
def segment_video(objects, original_image_state, fps=VIDEO_FPS):
    ...
    for obj_id, obj_data in objects.items():
        ...
        for point in obj_data["points"]:
            new_x, new_y = transform_point(
                (point[0], point[1]), (obj_data["mask"][0][0].shape)
            )
            obj_data["new_points"].append((int(new_x), int(new_y), point[2]))
        np_points = np.array(
            [[p[0], p[1]] for p in obj_data["new_points"]], dtype=np.float32
        )
        labels = np.array(
            [1 if p[2] == "positive_point" else 0 for p in obj_data["new_points"]],
            dtype=np.int32,
        )
    ...
    obj_mask_dir = os.path.join(temp_dir, "masks")
    ...
    for obj_id in objects.keys():
        extracted_video_paths[obj_id] = os.path.join(
            video_dir, f"extracted_video_obj_{obj_id}.mp4"
        )
        object_writers[obj_id] = cv2.VideoWriter(
            extracted_video_paths[obj_id],
            cv2.VideoWriter_fourcc(*"mp4v"),
            fps,
            (width, height),
        )

    for out_frame_idx, out_obj_ids, out_mask_logits in predictor_sam.propagate_in_video(
        inference_state
    ):
...
def estimate_depth_intrinsic(
    grid_size, vo_points, fps, original_image_state, processing_mode
):
    ...
    try:
        ...
        print(
            f"📊 Parameters: grid_size={grid_size}, vo_points={vo_points}, fps={fps}, mode={processing_mode}"
        )
...
def launch_viz(original_image_state):
    ...
    try:
        ...
        if os.path.exists(npz_path):
            ...
            return (
                iframe_html,
                track2d_video if os.path.exists(track2d_video) else None,
                html_path,
            )
        else:
...
def generate_final_mask(
    seg_path, depth_path, out_path, area_threshold=100, max_iter=50
):
    """
    ...
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
        occluder_mask_bbox, connectivity=8
    )
...
def generate_model_and_rescale_model(
    ...
    rgb_dir = os.path.join(temp_dir, "rgb")
    rgb_names = [
        os.path.join(rgb_dir, f) for f in os.listdir(rgb_dir) if f.endswith(".jpg")
    ]
    rgb_names.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))

    depth_dir = os.path.join(temp_dir, "depth")
    depth_names = [
        os.path.join(depth_dir, f) for f in os.listdir(depth_dir) if f.endswith(".png")
    ]
    ...
    if is_occluded:
        final_mask = generate_final_mask(
            mask_names[0], depth_names[0], model_dir, area_threshold=100, max_iter=50
        )
    else:
    ...
    video_path, mesh_path, high_mesh_path = generate_3d(
        rgb_image,
        final_mask,
        temp_dir,
        "obj",
    ...
    scaled_model_path, scale, anchor_pose = recover_true_scale(
        mesh_path, depth_names[0], intrinsic, rgb_names[0], mask_names[0], model_dir
    )
...
def generate_3d(
    ...
    slat_guidance_strength: float = 15,
...
def recover_true_scale(
    normal_model_path: str,
    anchor_depth_name: list,
    anchor_intrinsic: list,
...
def save_slat(slat, save_path: str):
    ...
    np.savez(save_path, feats=feats_numpy, coords=coords_numpy)
...
with gr.Blocks(
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            reset_points_btn = gr.Button(
                "🔄 Reset Selection", variant="secondary", size="sm"
            )
            is_occluded = gr.Checkbox(
                label="🎭 Object is Partially Occluded", value=False
            )
    ...
    with gr.Row():
        with gr.Column(scale=3):
            launch_btn = gr.Button(
                "🚀 Start Pipeline Now!", variant="primary", size="lg"
            )
        with gr.Column(scale=1):
        ...
        with gr.Accordion(label="Generation Settings", open=False):
            ...
            with gr.Row():
                slat_guidance_strength = gr.Slider(
                    0.0, 10.0, label="Guidance Strength", value=3, step=0.1
                )
    ...
    with gr.Row(visible=False):
        with gr.Column(scale=1):
            tracking_video_download = gr.File(
                label="📹 Download 2D Tracking Video", interactive=False, visible=False
            )
        with gr.Column(scale=1):
    ...
    launch_btn.click(
        fn=segment_video, inputs=[objects, original_image_state], outputs=[video_output]
    ).then(
